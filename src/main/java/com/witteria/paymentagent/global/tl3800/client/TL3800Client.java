...
public class TL3800Client implements AutoCloseable {
  ...
  public TLPacket requestResponse(TLPacket req) throws Exception {
    ...
    while (true) {
      ...
      Integer first = waitAckNakStx(tl3800Config.getAckWaitMs());
      if (first != null) {
        if (first == 0x15) { // NAK
          if (++tries <= tl3800Config.getMaxAckRetry()) {
            log.warn("[TL3800] << NAK → retry {}/{}", tries, tl3800Config.getMaxAckRetry());
            continue;
          }
      ...
      log.debug(
          "[TL3800] no-ACK within {} ms → waiting STX up to {} ms",
          tl3800Config.getAckWaitMs(),
          tl3800Config.getRespWaitMs());
      long start = System.currentTimeMillis();
      while ((System.currentTimeMillis() - start) < tl3800Config.getRespWaitMs()) {
        int b = t.readByte(50);
        if (b == 0x02) {
          byte[] header = readHeaderFromStx(); // 통일
          TLPacket p = readOrFollowUpIfEvent(header, expectedFinal, req);
          ...
          if (++tries <= tl3800Config.getMaxAckRetry()) {
            log.warn("[TL3800] << late NAK → retry {}/{}", tries, tl3800Config.getMaxAckRetry());
            break;
          }
          throw new IllegalStateException("NAK received (exceeded retry)");
        } else if (b == 0x06) {
  ...
  /**
   * ACK/즉시-STX/late-ACK 모두 여기로 통일: STX를 잡으면 단순히 34바이트를 더 읽어서 헤더로 사용 STX는 이미 읽힌 상태에서 호출된다고 가정하고, 나머지
   * 34바이트만 읽어 헤더 구성
   */
  ...
  private void consumeEventFrame(byte[] header) throws Exception {
    ...
    int m = t.readFully(tail, tailLen, tl3800Config.getRespWaitMs());
  ...
  private TLPacket waitResendAndReturnExpected(JobCode expected) throws Exception {
    ...
    while (System.currentTimeMillis() < deadline) {
      try {
        TLPacket pkt = readNextFrameAndAck(tl3800Config.getRespWaitMs());
  ...
  private byte[] composeHeaderAfterStxWithSliding() throws Exception {
    ...
    int n = t.readFully(rest, REST, tl3800Config.getRespWaitMs());

    if (n != REST) {
      log.warn(
          "[TL3800] header short: got={} need={} raw34={}", n, REST, hex(Arrays.copyOf(rest, n)));
      return null;
    }
    ...
    if (idx >= 0 && idx < REST - 1) {
      ...
      int m = t.readFully(more, missing, tl3800Config.getRespWaitMs());
      if (m != missing) {
        log.warn(
            "[TL3800] header2 short: got={} need={} rawMore={}",
            m,
            missing,
            hex(Arrays.copyOf(more, m)));
        return null;
      }
